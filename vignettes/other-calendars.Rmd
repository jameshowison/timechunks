...
The codes follow the industry-standard `Q1FY26` convention (lowercase:
`q1fy26`). A convenient side-effect: within a fiscal year the codes sort
alphabetically in the correct order (`q1fy < q2fy < q3fy < q4fy`).
Across years you still need `timechunks` chronological sorting, since
`q4fy26` would sort after `q1fy27` alphabetically.

```{r fy-parse}
# Parsing works the same way as semesters
time_chunk("q1fy26")   # Q1 starting Oct 2026
time_chunk("q2fy27")   # Q2 starting Jan 2027
time_chunk(202610)    # YYYYM: October 2026 → Q1
```

```{r fy-dates}
x <- time_chunk(c("q1fy26", "q2fy27", "q3fy27", "q4fy27"))
data.frame(
...
```{r fy-arithmetic}
# One quarter forward
time_chunk("q1fy26") + 1L   # Q2 (Jan 2027)

# Distance between quarters
time_chunk("q4fy27") - time_chunk("q1fy26")   # 3L — three quarters apart

# Full fiscal year sequence
seq(time_chunk("q1fy26"), time_chunk("q4fy27"))
```
...
revenue |>
  ...
  geom_col() +
  facet_grid(. ~ fy, scales = "free_x", space = "free_x") +
  labs(
